...
class _NewLogsBaseDataset(InMemoryDataset):
    """Shared body for new_logs/-derived datasets. Uses local pickles only."""
    NAME = None
    ...
    def download(self):
        raise RuntimeError(
            f"Raw pickles missing in {self.raw_dir}. Run GTconvertor.py first; "
            f"no remote download is configured for {self.name}.")
        ...
        for split in ['train', 'val', 'test']:
            ...
            for graph in graphs:
                data = Data(x=graph.x, edge_index=graph.edge_index,
                            edge_attr=graph.edge_attr, y=graph.y,
                            cid=graph.cid, pl=graph.pl)
                if self.pre_filter is not None and not self.pre_filter(data):
                    continue
                if self.pre_transform is not None:
                    data = self.pre_transform(data)
...
class EVENTHelpDeskV2(_NewLogsBaseDataset):
    NAME = "EVENTHelpDeskV2"


class EVENTBaseline(_NewLogsBaseDataset):
    NAME = "EVENTBaseline"


class EVENTBaselineDrifted(_NewLogsBaseDataset):
    NAME = "EVENTBaselineDrifted"


class EVENTBPIC2011(_NewLogsBaseDataset):
    NAME = "EVENTBPIC2011"


class EVENTBPIC2015(_NewLogsBaseDataset):
    NAME = "EVENTBPIC2015"


class EVENTProduction(_NewLogsBaseDataset):
    NAME = "EVENTProduction"


class EVENTPurchasing(_NewLogsBaseDataset):
    NAME = "EVENTPurchasing"


class EVENTBpi12w(_NewLogsBaseDataset):
    NAME = "EVENTBpi12w"


class EVENTConfidential(_NewLogsBaseDataset):
    NAME = "EVENTConfidential"


class EVENTBPIC20Prepaid(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20Prepaid"


class EVENTBPIC20InternationalV2(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20InternationalV2"


class EVENTBPIC20RFP(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20RFP"


class EVENTBPIC20Permit(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20Permit"


class EVENTBPIC20DomesticV2(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20DomesticV2"


class EVENTBPIC12V2(_NewLogsBaseDataset):
    NAME = "EVENTBPIC12V2"


class EVENTHospitalBilling(_NewLogsBaseDataset):
    NAME = "EVENTHospitalBilling"


class EVENTBPIC17(_NewLogsBaseDataset):
    NAME = "EVENTBPIC17"


class EVENTTrafficFinesV2(_NewLogsBaseDataset):
    NAME = "EVENTTrafficFinesV2"


class EVENTBPIC20IPaper(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20IPaper"


class EVENTBPIC20RFPPaper(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20RFPPaper"


class EVENTBPIC20PermitPaper(_NewLogsBaseDataset):
    NAME = "EVENTBPIC20PermitPaper"


# Legacy class kept for clarity; subclass not used for the rest:
class _EVENTHelpDeskV2_Legacy(InMemoryDataset):
    """Replaced by EVENTHelpDeskV2(_NewLogsBaseDataset). Kept for reference."""

    def __init__(self, root, split='train', transform=None, pre_transform=None,
                 pre_filter=None):
        self.name = "EVENTHelpDeskV2"
        assert split in ['train', 'val', 'test']
        super().__init__(root, transform, pre_transform, pre_filter)
        path = osp.join(self.processed_dir, f'{split}.pt')
        self.data, self.slices = torch.load(path)
    ...
    def download(self):
        raise RuntimeError(
            f"Raw pickles are missing in {self.raw_dir}. Run GTconvertor.py first; "
            "no remote download is configured for EVENTHelpDeskV2.")
...
class EVENTTrafficfinesABLATION(InMemoryDataset):
    ...
    def process(self):
        for split in ['train', 'val', 'test']:
            ...
            torch.save(self.collate(data_list),
                       osp.join(self.processed_dir, f'{split}.pt'))
class EVENTBpi11Half(_NewLogsBaseDataset):
    NAME = "EVENTBpi11Half"


class EVENTBpi11Calib(_NewLogsBaseDataset):
    NAME = "EVENTBpi11Calib"
