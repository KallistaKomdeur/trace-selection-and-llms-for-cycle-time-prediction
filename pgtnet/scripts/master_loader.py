...
# Achtung! The following code is added to the original implementation of GraphGPS framework
from graphgps.loader.dataset.GTeventlogHandler import (
    EVENTBPIC12, EVENTBPIC12W, EVENTBPIC12CW, EVENTBPIC12A, EVENTBPIC12O, EVENTBPIC12C,
    ...
    EVENTHelpDeskV2, EVENTBPIC20Prepaid, EVENTBPIC20InternationalV2,
    EVENTBPIC20RFP, EVENTBPIC20Permit, EVENTBPIC20DomesticV2,
    EVENTBPIC12V2, EVENTHospitalBilling, EVENTBPIC17, EVENTTrafficFinesV2,
    EVENTBPIC20IPaper, EVENTBPIC20RFPPaper, EVENTBPIC20PermitPaper, EVENTBaseline,
    EVENTBaselineDrifted, EVENTBPIC2011, EVENTBPIC2015)
# new logs (total-time target)
from graphgps.loader.dataset.GTeventlogHandler import (
    EVENTConfidential, EVENTProduction, EVENTPurchasing, EVENTBpi12w, EVENTBpi11Half)
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        elif pyg_dataset_id == 'EVENTHelpDesk':
            dataset = preformat_EVENTHelpDesk(dataset_dir)

        elif pyg_dataset_id == 'EVENTHelpDeskV2':
            dataset = preformat_EVENTHelpDeskV2(dataset_dir)
        elif pyg_dataset_id == 'EVENTProduction':
            dataset = preformat_EVENTProduction(dataset_dir)
        elif pyg_dataset_id == 'EVENTPurchasing':
            dataset = preformat_EVENTPurchasing(dataset_dir)
        elif pyg_dataset_id == 'EVENTBpi11Half':
            dataset = preformat_EVENTBpi11Half(dataset_dir)
        elif pyg_dataset_id == 'EVENTBpi12w':
            dataset = preformat_EVENTBpi12w(dataset_dir)
        elif pyg_dataset_id == 'EVENTConfidential':
            dataset = preformat_EVENTConfidential(dataset_dir)
        elif pyg_dataset_id == 'EVENTBaseline':
            dataset = preformat_EVENTBaseline(dataset_dir)
        elif pyg_dataset_id == 'EVENTBaselineDrifted':
            dataset = preformat_EVENTBaselineDrifted(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC2011':
            dataset = preformat_EVENTBPIC2011(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC2015':
            dataset = preformat_EVENTBPIC2015(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20Prepaid':
            dataset = preformat_EVENTBPIC20Prepaid(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20InternationalV2':
            dataset = preformat_EVENTBPIC20InternationalV2(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20RFP':
            dataset = preformat_EVENTBPIC20RFP(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20Permit':
            dataset = preformat_EVENTBPIC20Permit(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20DomesticV2':
            dataset = preformat_EVENTBPIC20DomesticV2(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC12V2':
            dataset = preformat_EVENTBPIC12V2(dataset_dir)
        elif pyg_dataset_id == 'EVENTHospitalBilling':
            dataset = preformat_EVENTHospitalBilling(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC17':
            dataset = preformat_EVENTBPIC17(dataset_dir)
        elif pyg_dataset_id == 'EVENTTrafficFinesV2':
            dataset = preformat_EVENTTrafficFinesV2(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20IPaper':
            dataset = preformat_EVENTBPIC20IPaper(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20RFPPaper':
            dataset = preformat_EVENTBPIC20RFPPaper(dataset_dir)
        elif pyg_dataset_id == 'EVENTBPIC20PermitPaper':
            dataset = preformat_EVENTBPIC20PermitPaper(dataset_dir)
...
def preformat_EVENTHelpDeskV2(dataset_dir):
    return join_dataset_splits([EVENTHelpDeskV2(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTProduction(dataset_dir):
    return join_dataset_splits([EVENTProduction(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTPurchasing(dataset_dir):
    return join_dataset_splits([EVENTPurchasing(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBpi12w(dataset_dir):
    return join_dataset_splits([EVENTBpi12w(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTConfidential(dataset_dir):
    return join_dataset_splits([EVENTConfidential(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBaseline(dataset_dir):
    return join_dataset_splits([EVENTBaseline(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBaselineDrifted(dataset_dir):
    return join_dataset_splits([EVENTBaselineDrifted(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC2011(dataset_dir):
    return join_dataset_splits([EVENTBPIC2011(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC2015(dataset_dir):
    return join_dataset_splits([EVENTBPIC2015(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20Prepaid(dataset_dir):
    return join_dataset_splits([EVENTBPIC20Prepaid(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20InternationalV2(dataset_dir):
    return join_dataset_splits([EVENTBPIC20InternationalV2(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20RFP(dataset_dir):
    return join_dataset_splits([EVENTBPIC20RFP(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20Permit(dataset_dir):
    return join_dataset_splits([EVENTBPIC20Permit(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20DomesticV2(dataset_dir):
    return join_dataset_splits([EVENTBPIC20DomesticV2(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC12V2(dataset_dir):
    return join_dataset_splits([EVENTBPIC12V2(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTHospitalBilling(dataset_dir):
    return join_dataset_splits([EVENTHospitalBilling(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC17(dataset_dir):
    return join_dataset_splits([EVENTBPIC17(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTTrafficFinesV2(dataset_dir):
    return join_dataset_splits([EVENTTrafficFinesV2(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20IPaper(dataset_dir):
    return join_dataset_splits([EVENTBPIC20IPaper(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20RFPPaper(dataset_dir):
    return join_dataset_splits([EVENTBPIC20RFPPaper(root=dataset_dir, split=s) for s in ['train','val','test']])

def preformat_EVENTBPIC20PermitPaper(dataset_dir):
    return join_dataset_splits([EVENTBPIC20PermitPaper(root=dataset_dir, split=s) for s in ['train','val','test']])
...
def preformat_EVENTBpi11Half(dataset_dir):
    return join_dataset_splits([EVENTBpi11Half(root=dataset_dir, split=s) for s in ['train','val','test']])
