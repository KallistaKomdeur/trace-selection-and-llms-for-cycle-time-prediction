...
# Split by explicit case-id lists instead of a ratio.
# The fixed-sets campaigns hand us a partition that is temporal on case *end* time,
# while train_test_split_func sorts by start time -- so no ratio can reproduce it.
# max_case_duration comes from the spec's own targets rather than from re-deriving
# case_end - case_start, so normalisation matches the targets exactly.
def explicit_split_func (log, event_log, split_spec):
    start_dates, end_dates = [], []
    for i in range (len(log)):
        current_case = log[i]
        start_dates.append(current_case[0].get('time:timestamp'))
        end_dates.append(current_case[len(current_case)-1].get('time:timestamp'))
    sorted_start_dates = tuple(sorted(start_dates))
    sorted_end_dates = tuple(sorted(end_dates))
    train_case_ids = [str(c) for c in split_spec['train']]
    validation_case_ids = [str(c) for c in split_spec['val']]
    test_case_ids = [str(c) for c in split_spec['test']]
    train_validation_case_ids = train_case_ids + validation_case_ids
    present = {str(log[i].attributes.get('concept:name')) for i in range(len(log))}
    missing = [c for c in train_validation_case_ids + test_case_ids if c not in present]
    if missing:
        raise SystemExit(f"explicit split names {len(missing)} case ids absent from the log, "
                         f"e.g. {missing[:5]}")
    case_spec = split_spec['case_spec']
    max_case_duration = max(case_spec[c]['target_days'] for c in train_validation_case_ids)
    def _filter (ids):
        return pm4py.filter_trace_attribute_values(event_log, 'case:concept:name', ids,
                                                   case_id_key='case:concept:name')
    training_dataframe = _filter(train_case_ids)
    validation_dataframe = _filter(validation_case_ids)
    test_dataframe = _filter(test_case_ids)
    train_validation_dataframe = _filter(train_validation_case_ids)
    training_event_log = pm4py.convert_to_event_log(training_dataframe)
    validation_event_log = pm4py.convert_to_event_log(validation_dataframe)
    test_event_log = pm4py.convert_to_event_log(test_dataframe)
    train_validation_event_log = pm4py.convert_to_event_log(train_validation_dataframe)
...
def graph_conversion_func (split_log, removed_cases, idx, data_list, case_attributes, case_encoder_list,
                           ...
                           target_normalization, max_time_norm, target_variable,
                           ...
                           event_avg_num_list, max_prefix_length=None, case_spec=None):
    ...
    for case_counter in range(len(split_log)):
        ...
        case_length = len(current_case)
        # A case with an explicit prefix list contributes exactly those prefixes, so the
        # usual "needs at least 3 events" rule does not apply to it: the fixed sets ask
        # for prefixes of length 2 on a sizeable share of the test cases.
        spec = (case_spec or {}).get(str(case_id))
        forced_prefixes = spec.get('prefix_lengths') if spec else None
        if case_length < 3 and not forced_prefixes:
            ...
            for att_index in range(len(case_num_att)):
                ...
                _rng = max_num_list[att_index] - min_num_list[att_index]
                if _rng == 0:
                    case_att_normalized = 0.0
                elif np.isnan(case_att):
                    case_att_normalized = (avg_num_list[att_index] - min_num_list[att_index]) / _rng
                else:
                    case_att_normalized = (case_att - min_num_list[att_index]) / _rng
            ...
            # max_prefix_length caps how many prefixes each case contributes. case_start and
            # case_end still come from the whole case, so the total-time target is unchanged:
            # only the number of observation points per case is reduced.
            prefix_limit = case_length if max_prefix_length is None else min(case_length, max_prefix_length + 1)
            prefix_range = forced_prefixes if forced_prefixes else range (2, prefix_limit)
            for prefix_length in prefix_range:
                ...
                # target_variable selects what is predicted:
                #   'remaining_time' (default, original PGTNet behaviour): time from the end of
                #                    the prefix to the end of the case.
                #   'total_time'   : the whole case cycle time, i.e. end of the case minus the
                #                    first event, which is the same quantity the LLM baselines
                #                    are asked to predict. Predicting remaining time and adding
                #                    the known elapsed time afterwards is not equivalent: it
                #                    leaves the residuals of a remaining-time model while scoring
                #                    them against the larger variance of the total time, which
                #                    inflates R^2.
                if spec is not None and spec.get('target_days') is not None:
                    # Target supplied by the caller (fixed-sets campaigns): the test cases
                    # are truncated prefixes, so case_end is the end of the prefix and not
                    # the end of the case -- deriving the target from it would be wrong.
                    if target_variable != 'total_time':
                        raise SystemExit("case_spec targets are total case time; "
                                         "set target_variable: total_time")
                    target_days = spec['target_days']
                elif target_variable == 'total_time':
                    target_days = (case_end - case_start).total_seconds()/3600/24
                else:
                    target_days = (case_end - collection_lists[1][prefix_length-1]).total_seconds()/3600/24
                if target_normalization:
                    target_cycle = np.array(target_days/max_time_norm)
                else:    
                    target_cycle = np.array(target_days)
                ...
                # One pass over the prefix records, for every directly-follows pair, the
                # positions where it occurs. The original code re-scanned the whole prefix
                # twice per edge and then took a cross product, which is O(L^3) per case and
                # made a 785-event trace cost minutes. Same pairs, same order, same
                # positions -- only the way they are found changes.
                class_index = {}
                for _ci, _c in enumerate(prefix_classes):
                    class_index[_c] = _ci
                pair_positions = {}
                for _x in range(prefix_length - 1):
                    _key = (class_index[prefix_event_classes[_x]],
                            class_index[prefix_event_classes[_x + 1]])
                    pair_positions.setdefault(_key, []).append((_x, _x + 1))
                edges_list = list(pair_positions.keys())
                ...
                for edge in edge_index:
                    acceptable_indices = pair_positions[(int(edge[0]), int(edge[1]))]
                    ...
                    for attribute_counter in range (len(event_attributes)+2,len(event_attributes)+len(event_num_att)+2):
                        ...
                        _i = attribute_counter - len(event_attributes) - 2
                        _rng = event_max_num_list[_i] - event_min_num_list[_i]
                        if _rng == 0:
                            norm_att_val = 0.0
                        elif np.isnan(attribute_value):
                            norm_att_val = (event_avg_num_list[_i] - event_min_num_list[_i]) / _rng
                        else:
                            norm_att_val = (attribute_value - event_min_num_list[_i]) / _rng
...
def main(directory, yml_file, overwrite):
    try:
        ...
        # 'remaining_time' keeps the original PGTNet target; 'total_time' predicts the full cycle
        target_variable = user_inputs.get('target_variable', 'remaining_time')
        max_prefix_length = user_inputs.get('max_prefix_length', None)
        # When set, this JSON drives both the partition and the per-case targets/prefixes,
        # and train_val_test_ratio is ignored.
        fixed_split_file = user_inputs.get('fixed_split_file', None)
        split_spec = None
        if fixed_split_file:
            with open(fixed_split_file) as _f:
                split_spec = json.load(_f)
            print(f'[GTconvertor] explicit split from {fixed_split_file}: '
                  f"{len(split_spec['train'])} train / {len(split_spec['val'])} val / "
                  f"{len(split_spec['test'])} test cases")
        print(f'[GTconvertor] target_variable = {target_variable}')
        ...
        if split_spec is not None:
            train_df, val_df, test_df, train_val_df, train_log, val_log,test_log,\
                train_val_log, max_time_norm, sorted_start_dates,\
                    sorted_end_dates = explicit_split_func(log, event_log, split_spec)
        else:
            train_df, val_df, test_df, train_val_df, train_log, val_log,test_log,\
                train_val_log, max_time_norm, sorted_start_dates,\
                    sorted_end_dates = train_test_split_func(log, event_log, split_ratio)
        case_spec = split_spec['case_spec'] if split_spec is not None else None
        print(f'[GTconvertor] max_time_norm = {max_time_norm}')
        ...
        removed_cases, idx, data_list = graph_conversion_func (train_log, removed_cases, idx, data_list, 
                                                               ...
                                                               max_prefix_length=max_prefix_length, case_spec=case_spec)
        last_train_idx = idx
